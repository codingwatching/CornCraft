...
namespace CraftSharp.Protocol.ProfileKey
{
    ...
    public static class KeysCache
    {
        ...
        private static bool LoadFromDisk()
        {
            ...
            if (File.Exists(KeysCacheFilePlaintext))
            {
                if (ProtocolSettings.DebugMode)
                    Debug.Log(Translations.Get("cache.loading_keys", KeysCacheFilePlaintext));
                        ...
                        if (!line.TrimStart().StartsWith("#"))
                        {
                            ...
                            if (separatorIdx >= 1 && line.Length > separatorIdx + 1)
                            {
                                ...
                                try
                                {
                                    ...
                                    keys[login] = playerKeyPair;
                                    if (ProtocolSettings.DebugMode)
                                        Debug.Log(Translations.Get("cache.loaded_keys", playerKeyPair.ExpiresAt));
                                }
                                catch (InvalidDataException e)
                                {
                                    if (ProtocolSettings.DebugMode)
                                        Debug.Log(Translations.Get("cache.ignore_string_keys", value, e.Message));
                                }
                            ...
                            {
                                Debug.Log(Translations.Get("cache.ignore_line_keys", line));
                            }
                        }
                    }
                }
                ...
                {
                    Debug.Log(Translations.Get("cache.read_fail_plain_keys", e.Message));
                }
            }
        ...
        {
            if (ProtocolSettings.DebugMode)
                Debug.Log(Translations.Get("cache.saving_keys"));

            List<string> KeysCacheLines = new()
            {
                "# Generated by CornClient " + ProtocolSettings.Version + " - Edit at own risk!",
            ...
            {
                Debug.Log(Translations.Get("cache.save_fail_keys", e.Message));
            }
        }
    }
}