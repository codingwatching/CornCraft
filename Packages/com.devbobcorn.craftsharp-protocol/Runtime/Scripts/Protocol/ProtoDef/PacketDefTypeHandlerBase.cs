...
namespace CraftSharp.Protocol.ProtoDef
{
    public abstract class PacketDefTypeHandlerBase
    {
        ...
        public static readonly ResourceLocation ARRAY_WITH_LENGTH_OFFSET_ID    = new("arrayWithLengthOffset"); // Added in 1.20.5
        ...
        private static PacketDefTypeHandlerBase BuildDefTypeHandler(string? scope, ResourceLocation customTypeId, ResourceLocation underlyingTypeId, JToken? typeParams, JObject typeDict)
        {
            PacketDefTypeHandlerBase buildItemHandler(JToken itemToken)
            {
                ...
                {
                    // This is a custom type without type parameters, use proxied handler
                    // to allow referencing handlers which are not yet completely declared
                    // as a part of this type, this allows cyclic dependency.
                    // This is used in Minecraft 1.20.5+ ProtoDef files by types such as
                    // EffectDetail, which contains an optional member of EffectDetail.
                    // See https://github.com/PrismarineJS/minecraft-data/blob/master/data/pc/1.20.5/protocol.json#L1184-L1215
